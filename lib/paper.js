...
var Raster = this.Raster = PlacedItem.extend({
	...
	hitTest : function(point, nodes, offset) {
		...
		for(i = 0; i < length; i++) {
			points.r = nodes[i].position.x + nodes[i].size.width - offset;
			points.l = nodes[i].position.x - nodes[i].size.width + offset;
			points.t = nodes[i].position.y + nodes[i].size.height - offset;
			points.b = nodes[i].position.y - nodes[i].size.height + offset;
